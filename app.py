...
production_days = st.sidebar.number_input("生产周期（天）", min_value=0, max_value=60, value=6)
shipping_days = st.sidebar.number_input("运输周期（天）", min_value=0, max_value=60, value=12)
safety_days = st.sidebar.number_input("安全库存天数", min_value=0, max_value=60, value=12)
...
if st.button("🚀 点击生成分析报表") and this_week_file and last_week_file:
    ...
    # ========== 补货建议主表 ==========
    restock_table = summary_df[[
        "Sold Count", "Last Week Sold Count", "Growth Rate", "Daily Avg",
        "Growth Multiplier", "Restock Qty", "未来赠送量", "补货总量含赠送",
        "当前库存", "最终补货量"
    ]]
